...
int main() {
    ...
    if (pcre2_jit_compile(re, PCRE2_JIT_COMPLETE)) {
        cout << "PCRE JIT compilation failed" << endl;
        pcre2_code_free(re);
        return -1;
    }
    jit_stack = pcre2_jit_stack_create(65536, 65536, NULL);
    if (!jit_stack) {
        cout << "PCRE JIT cannot allocate JIT stack" << endl;
        pcre2_code_free(re);
        return -1;
    }
